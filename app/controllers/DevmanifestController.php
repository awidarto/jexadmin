...
//device=&courier=&logistic=&date-from=2015-10-24
        ...
        */
        ...
        foreach ($actualresult as $r) {
            ...
            $details = Deliverydetail::where('delivery_id','=',$r->delivery_id)
            ...
            $tabdata[] = array(
                    ...
                    array('value'=>$r->delivery_type,'attr'=>'class="currency '.$codclass.'" '),
                    array('value'=>$r->status.'<br /><br />'.$r->pickup_status.'<br /><br />'.$r->warehouse_status,'attr'=>''),
                    array('value'=>$r->buyer_name,'attr'=>''),
                    array('value'=>( $payable == 0 )?0:Ks::idr($payable),'attr'=>'class="currency '.$codclass.'" '),
                    array('value'=>( $dc == 0 )?0:Ks::idr($dc),'attr'=>'class="currency '.$codclass.'" style="position:relative;"'),
                    array('value'=>( $cod == 0 )?0:Ks::idr($cod),'attr'=>'class="currency '.$codclass.'" style="position:relative;"'),
                    array('value'=>( $chg == 0 )?0:Ks::idr($chg),'attr'=>'class="currency '.$codclass.'" '),
                    array('value'=>$r->shipping_address,'attr'=>''),
                    array('value'=>'<span '.$phone_dupe.' >'.$this->split_phone($r->phone).'<br />'.$this->split_phone($r->mobile1).'<br />'.$this->split_phone($r->mobile2).'</span>','attr'=>''),
                    array('value'=>$this->hide_trx($r->merchant_trans_id).$fcode.'<br/>'.$this->date_did($r->delivery_id),'attr'=>'class="currency cod"'),
                    array('value'=>'','attr'=>''),
                    array('value'=>'','attr'=>''),
                );
